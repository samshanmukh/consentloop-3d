...
The patient UI now uses Deepgram's unified Voice Agent API for an interruption-friendly live conversation. A user must explicitly start the microphone. The persistent glass voice dock then shows real connection, listening, thinking, and speaking states; supports barge-in; displays live conversation text; accepts typed questions as an accessible alternative; and can be stopped at any time. The right-knee walkthrough is deterministic: whole body, settled knee highlight, settled zoom/detail handoff, and then one configured procedure step per narration turn. Batched, skipped, rewound, or premature-completion calls are rejected, and the next visual cannot start until the prior narration finishes playing or the patient interrupts it.
